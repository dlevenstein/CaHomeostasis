figfolder  = '/Users/dl2820/Project Repos/CaHomeostasis/DailyNotebook/Notebook20210827';
...
A = 50;     %Immediate post-TTX rate
tau = 25.*60;   %Decay time constant
f = 40.*60;     %Presynaptic oscillation frequency
phi = 7.*60;
...
subplot(4,2,1)
    hold on
    plot(simresults.t_hr,simresults.b.*simresults.v,'color','k','linewidth',2)
    plot(simresults_TS.t_hr,simresults_TS.b.*simresults_TS.v,'color','r','linewidth',2)
    ylabel('bCamKII')
    xlim([-5 72])
    box off
    xlabel('T (hr)')
...
subplot(4,2,3)
    hold on
    plot(simresults.t_hr,(1-simresults.b).*simresults.v,'color','k','linewidth',2)
    plot(simresults_TS.t_hr,(1-simresults_TS.b).*simresults_TS.v,'--','color','r','linewidth',2)
    ylabel('aCamKII')
    xlim([-5 72])
    box off
    xlabel('T (hr)')
